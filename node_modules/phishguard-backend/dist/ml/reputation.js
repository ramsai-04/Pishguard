...
const getDomainAgeDays = async (domain, timeoutMs) => {
...
const checkGoogleSafeBrowsing = async (normalizedUrl, timeoutMs) => {
    if (!env.GOOGLE_SAFE_BROWSING_API_KEY) {
        return { checked: false, isMalicious: false };
    }
    const endpoint = `https://safebrowsing.googleapis.com/v4/threatMatches:find?key=${encodeURIComponent(env.GOOGLE_SAFE_BROWSING_API_KEY)}`;
    const body = {
        client: {
            clientId: "phishguard",
            clientVersion: "1.0.0",
...
const checkPhishTank = async (normalizedUrl, timeoutMs) => {
    ...
    const data = await fetchJsonWithTimeout(env.PHISHTANK_ENDPOINT, {
        method: "POST",
...
const checkTlsCertificate = (domain, timeoutMs) => new Promise((resolve) => {
...
export const evaluateDomainReputation = async (input) => {
    ...
    if (env.REPUTATION_ENABLE_EXTERNAL) {
        ...
        if ((gsb.checked || phishTank.checked) &&
            !gsb.isMalicious &&
