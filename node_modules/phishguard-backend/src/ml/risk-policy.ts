...
const BRAND_RULES: Array<{ brand: string; legitDomains: string[] }> = [
...
const clip01 = (value: number): number => Math.max(0, Math.min(1, value));
...
const domainContainsBrandSpoof = (domain: string, url: string): string[] => {
  const reasons: string[] = [];
  for (const rule of BRAND_RULES) {
    if (!url.includes(rule.brand)) continue;
...
export const applyRiskPolicy = (input: PolicyInput): PolicyOutput => {
