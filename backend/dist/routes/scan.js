...
const TRUSTED_DOMAINS = env.TRUSTED_DOMAINS.split(",")
    ...
    .filter(Boolean);
const isTrustedDomain = (domain) => TRUSTED_DOMAINS.some((trusted) => domain === trusted || domain.endsWith(`.${trusted}`));
const isBlockedEntryExpired = (lastSeen) => {
...
scanRouter.post("/", requireAuth, async (req, res, next) => {
    try {
        ...
        const canConsiderBlocking = isPhishing &&
            !trustedDomain &&
        ...
        if (canConsiderBlocking) {
            ...
            if (detections >= env.PHISHING_BLOCKLIST_MIN_DETECTIONS &&
