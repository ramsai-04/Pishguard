...
export const RF_FEATURE_ORDER: Array<keyof UrlFeatures> = [
  "urlLength",
  "domainLength",
  "isDomainIP",
  "tldLength",
...
export const detectCategory = (url: string): string => {
  const lowerUrl = url.toLowerCase();
  if (/(amazon|flipkart|shop|store|buy|cart)/.test(lowerUrl)) return "E-commerce";
  if (/(coursera|udemy|edu|learn|school|course)/.test(lowerUrl)) return "Education";
  if (/(health|medical|doctor|hospital|clinic)/.test(lowerUrl)) return "Health";
  if (/(bank|paypal|chase|finance|credit|wallet)/.test(lowerUrl)) return "Banking";
  if (/(gov|government|irs|passport)/.test(lowerUrl)) return "Government";
  if (/(netflix|youtube|movie|music|stream)/.test(lowerUrl)) return "Entertainment";
  if (/(facebook|twitter|instagram|social|linkedin)/.test(lowerUrl)) return "Social Media";
  if (/(google|microsoft|tech|software|cloud|github)/.test(lowerUrl)) return "Technology";
  return "Other";
};

export const extractFeatures = (rawUrl: string): UrlFeatures => {
...
export const vectorizeFeatures = (features: UrlFeatures): number[] =>
  RF_FEATURE_ORDER.map((key) => features[key]);
